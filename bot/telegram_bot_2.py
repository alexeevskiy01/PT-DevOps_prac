...
def writing_phone_numbers_to_db(update: Update, context):
    ...
    if user_input == 'ДА' or user_input == 'Да' or user_input == 'да':
        if set_information_to_database(table='phonenumbers (number)', dotenv_path=dotenv_path, values=numbersList):
...
def writing_emails_to_db(update: Update, context):
    ...
    user_input = update.message.text
    if user_input == 'ДА' or user_input == 'Да' or user_input == 'да':
        if set_information_to_database(table='emails (email)', dotenv_path=dotenv_path, values=emailList):
            update.message.reply_text('Данные успешно записаны')
        else:
            update.message.reply_text('Ошибка записи')
...
def get_information_from_remote_server(command: str, dotenv_path: Path) -> str:
...
def get_release(update: Update, context):
    update.message.reply_text(get_information_from_remote_server(command='lsb_release -a', dotenv_path=dotenv_path))


def get_uname(update: Update, context):
    update.message.reply_text(get_information_from_remote_server(command='uname -a', dotenv_path=dotenv_path))


def get_uptime(update: Update, context):
    update.message.reply_text(get_information_from_remote_server(command='uptime', dotenv_path=dotenv_path))


def get_df(update: Update, context):
    update.message.reply_text(get_information_from_remote_server(command='df', dotenv_path=dotenv_path))


def get_free(update: Update, context):
    update.message.reply_text(get_information_from_remote_server(command='free -h', dotenv_path=dotenv_path))


def get_mpstat(update: Update, context):
    update.message.reply_text(get_information_from_remote_server(command='mpstat', dotenv_path=dotenv_path))


def get_w(update: Update, context):
    update.message.reply_text(get_information_from_remote_server(command='w', dotenv_path=dotenv_path))


def get_auths(update: Update, context):
    update.message.reply_text(get_information_from_remote_server(command='last -10', dotenv_path=dotenv_path))


def get_critical(update: Update, context):
    update.message.reply_text(
        get_information_from_remote_server(command='journalctl -r -p crit -n 5', dotenv_path=dotenv_path))


def get_ps(update: Update, context):
    update.message.reply_text(get_information_from_remote_server(command='ps -ef | head -n 15', dotenv_path=dotenv_path))


def get_ss(update: Update, context):
    update.message.reply_text(get_information_from_remote_server(command='ss -lntu', dotenv_path=dotenv_path))


def get_services(update: Update, context):
    update.message.reply_text(
        get_information_from_remote_server(
            command='systemctl list-units --type=service | head -n 15', dotenv_path=dotenv_path))
...
def get_apt_list(update: Update, context):
    ...
    update.message.reply_text(
            get_information_from_remote_server(command=command, dotenv_path=dotenv_path))
...
def get_repl_logs(update: Update, context):
    ...
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    client.connect(hostname=host, username=username, password=password, port=port)
    stdin, stdout, stderr = client.exec_command("docker exec -i db_container cat /log/postgresql.log | grep 'replic' ")
    data = stdout.read() + stderr.read()
...
def get_emails(update: Update, context):
    update.message.reply_text(get_information_from_database(table='emails', dotenv_path=dotenv_path))


def get_phone_numbers(update: Update, context):
    update.message.reply_text(get_information_from_database(table='phonenumbers', dotenv_path=dotenv_path))


def get_information_from_database(table: str, dotenv_path: Path):
    load_dotenv(dotenv_path=dotenv_path)
...
def set_information_to_database(table: str, dotenv_path: Path, values: list) -> bool:
    load_dotenv(dotenv_path=dotenv_path)
    flag = False
